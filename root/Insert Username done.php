...
if (isset($_POST['submit_1'])) {
	...
	if (empty($I_username) || empty($Tel_no) || empty($ID_User) || empty($Address)) {
		...
		echo "<br><font color='red'>Please fill in all the information!</font>";
		exit();
	} else {
		...
		if ($resultCheck > 0) {
			echo "<br><font color='red'>User already exists!</font>";
			exit();
		} else {
			$sql = "SELECT * FROM users WHERE Tel_no='$Tel_no'";
		$result_1 = mysqli_query($conn, $sql);
		$resultCheck_1 = mysqli_num_rows($result_1);
		
		if ($resultCheck_1 > 0) {
			echo "<br><font color='red'>Telephone number already exists!</font>";
			exit();
		} else {
		$sql = "INSERT INTO users (Name,Tel_no,ID,Address) VALUES ('$I_username', '$Tel_no', '$ID_User', '$Address');";
		$result = mysqli_query($conn, $sql);
